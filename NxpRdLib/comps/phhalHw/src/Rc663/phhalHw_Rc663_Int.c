...
unsigned short bRC663_T0LD = 0;
unsigned short bRC663_T1LD = 0;
/*************寄存器索引*************/
static const uint8_t PH_MEMLOC_CONST_ROM gkphhalHw_Rc663_Registers[] =
{
    ...
    PHHAL_HW_RC663_REG_TXDATANUM,//发送数据编号
    PHHAL_HW_RC663_REG_TXMODWIDTH,//调制脉宽
    PHHAL_HW_RC663_REG_TXSYM10BURSTLEN,//发送1和0的间隔
    PHHAL_HW_RC663_REG_TXWAITCTRL,//发送时间等待控制，数据发送完成之后，在这段直接内，忽略引脚接收的任何数据
...
uint8_t PH_MEMLOC_CONST_ROM gkphhalHw_Rc663_I14443A_106[] =
{
    0x8E,  //  丛 0x28 寄存器开始   PHHAL_HW_RC663_REG_DRVMODE
    0x15,
    0x11,
    0x01,//  0x01,//0x0A,
    0x18,
...
uint8_t PH_MEMLOC_CONST_ROM gkphhalHw_Rc663_I14443B_106[] =
{
    0x8F,
    PHHAL_HW_RC663_MODINDEX_I14443B,// 0x0c,//0x00 | PHHAL_HW_RC663_MODINDEX_I14443B,
    0x01,//0x71,//0x01,
    0x0A,// 0x01,// 0x0A,
    0x7B,
    0x7B,//CRC   A:0x18  bit0:是否时能修改CRC错误标识;bit:是否CRC写入FIFO
    //0xfB,
    0x08,
    0x00,
    0x00,
                                 ...
                                 )
{
    ...
    /* Read out the lower FiFo Level bits */
    bValue = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_FIFOLENGTH << 1) | 0x01U);
    wFifoLen = (uint16_t)bValue;

    /* Read out the higher FiFo Level bits if applicable */
    if ((bValue == 0x00) && (pDataParams->bFifoSize == PHHAL_HW_RC663_VALUE_FIFOSIZE_512)) {
        bValue = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_FIFOCONTROL << 1) | 0x01U);
        ///phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_FIFOCONTROL, &bValue);
    ...
    if (*pLength > 0) {
        phhalHw_Rc663_ReadData(pDataParams, PHHAL_HW_RC663_REG_FIFODATA, *pLength, pData);
    }
                                  ...
                                  )
{
    ...
    SPI_RC663_Write( (PHHAL_HW_RC663_REG_FIFOCONTROL << 1),bValue);
                                 ...
                                 )
{
    ...
    if (pDataParams->bBalConnectionType == PHHAL_HW_BAL_CONNECTION_SPI) {
        ...
        for (wIndex = 0; wIndex < wLength; wIndex++) {
            // phhalHw_ReadRegister(pDataParams, bAddress, &pData[wIndex]);
            pData[wIndex] = SPI_RC663_Read( (uint8_t)(bAddress << 1) | 0x01U);
        }
    }
    /* I2C protocol */
    else if (pDataParams->bBalConnectionType == PHHAL_HW_BAL_CONNECTION_I2C) {
...
/**************************************************************
*设置接收等待时间，发送之后，如果在这时间之内，收到了数据，就忽视掉
***************************************************************/
...
/**************************************************************
* 
* 说明：设置发送等待时间，发送完成之后，在这段时间内，忽视所有接收到的数据
***************************************************************/
...
/**************************************************************
*说明：设置帧的延时时间 为，采用了两个定时器，分为为T0和T1,T0为地位，T1为高位置； 
*       当时间单位为ms的时候，T0没1MS就溢出，给T1产生一个加1；单位US的时候，如果定时超出了
*       T0的分为，T0地位为ETU就溢出； 
*       bUnit--时间单位
*       当地位为US的时候，T0使用ETU溢出，最多只能但是65536*9.44US,
***************************************************************/
                               ...
                               )
{
    ...
    if (bUnit == PHHAL_HW_TIME_MICROSECONDS) {//单位为US
...
//      else
        {
            /* T0 Overflow occurs at exactly 1 ETU (from ISO14443 protocol @ 106) *///T0 是1个ETU一出一次
    ...
    /* Calculate values for Millisecond values */
        ...
        wT0Reload = (uint16_t)(1000.0f * PHHAL_HW_RC663_TIMER0_FREQ);//定时器T0没1ms
    ...
    // phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_T0RELOADHI, (uint8_t)(wT0Reload >> 8));

    // phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_T0RELOADLO, (uint8_t)(wT0Reload));
    SPI_RC663_Write( (PHHAL_HW_RC663_REG_T0RELOADHI << 1),(uint8_t)(wT0Reload >> 8));
    SPI_RC663_Write( (PHHAL_HW_RC663_REG_T0RELOADLO << 1),(uint8_t)(wT0Reload));

    /* Write T1 reload value */
    //phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_T1RELOADHI, (uint8_t)(wT1Reload >> 8));
    // phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_T1RELOADLO, (uint8_t)(wT1Reload));

    SPI_RC663_Write( (PHHAL_HW_RC663_REG_T1RELOADHI << 1),(uint8_t)(wT1Reload >> 8));
    SPI_RC663_Write( (PHHAL_HW_RC663_REG_T1RELOADLO << 1),(uint8_t)(wT1Reload));


    bRC663_T0LD = wT0Reload ;
    bRC663_T1LD = wT1Reload;
...
/**************************************************************
*说明：获取帧的保护时间，地位为us,时间为30US,更改了浮点计算，现在使用20us，没有更改事前使用150us 
* 
***************************************************************/
...
//  /* Get T0 reload value */
//  phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T0RELOADHI, &bRegister);
//  wT0Reload = (uint16_t)bRegister << 8;
//  phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T0RELOADLO, &bRegister);
//  wT0Reload |= (uint16_t)bRegister;
//
//  /* Get T1 reload value */
//  phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T1RELOADHI, &bRegister);
//  wT1Reload = (uint16_t)bRegister << 8;
//  phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T1RELOADLO, &bRegister);
//  wT1Reload |= (uint16_t)bRegister;

    wT0Reload = bRC663_T0LD ;
    wT1Reload = bRC663_T1LD ;

    /* Get T0 counter value */
    if ((wExchangeStatus & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT) {
        //phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T0COUNTERVALHI, &bRegister);
        bRegister = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_T0COUNTERVALHI << 1) | 0x01U);
        wT0Value = (uint16_t)bRegister << 8;
        //phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T0COUNTERVALLO, &bRegister);
        bRegister = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_T0COUNTERVALLO << 1) | 0x01U);
    ...
    // phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T1COUNTERVALHI, &bRegister);
    bRegister = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_T1COUNTERVALHI << 1) | 0x01U);
    wT1Value = (uint16_t)bRegister << 8;
    // phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T1COUNTERVALLO, &bRegister);
    bRegister = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_T1COUNTERVALLO << 1) | 0x01U);
    ...
    // fTime = (float32_t)wT1Value / (PHHAL_HW_RC663_TIMER0_FREQ / (float32_t)wT0Reload);//10us
    // fTime = fTime + ((float32_t)wT0Value / PHHAL_HW_RC663_TIMER0_FREQ);//10us

    fTime = (float32_t)(wT1Value*wT0Reload) / (PHHAL_HW_RC663_TIMER0_FREQ );
    ...
    *pTime = (uint32_t)fTime;
    ...
    if ((wExchangeStatus & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT) {
        phhalHw_Rc663_GetDigiDelay(pDataParams, PH_OFF, &wDelay);
        *pTime = *pTime - wDelay;
    }
...
* 
* 获取缓冲区的长度；
* pTxBuffer：    指向缓冲区有效的指针
* pTxBufferSize：缓冲区整个大小：
* pTxBufferLen： 缓冲区有效数据的长度
* 
* 
***************************************************************/
...
phStatus_t phhalHw_Rc663_GetRxBuffer(
                                    phhalHw_Rc663_DataParams_t * pDataParams,
                                    uint8_t bIsExchange,
                                    uint8_t ** pRxBuffer,//接收缓冲区指针，指向当前空闲的位置
                                    uint16_t * pRxBufferLen,//接收缓冲区的有效字节的长度,里面包括了上次接收链接I块的数据
                                    uint16_t * pRxBufferSize//接收缓冲区的大小
                                    )
{
