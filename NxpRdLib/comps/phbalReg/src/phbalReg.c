...
//void Print_LogHex(void *str0,unsigned char *hex,unsigned int hexlen){
//    unsigned int i;
//    printf("%s",str0);
//    for(i = 0; i< hexlen; i++)
//        printf("%02x ",hex[i]);//(*((unsigned char *)(hex+i))));//[i]);//((unsigned char *)hex)&(hex+i)));
//    printf("\n\r");
//}
...
//  }
    ...
    return phbalReg_Stub_Exchange((phbalReg_Stub_DataParams_t*)pDataParams, wOption, pTxBuffer, wTxLength, wRxBufSize, pRxBuffer, pRxLength);
    /* perform operation on active layer */
    switch (PH_GET_COMPID(pDataParams))
    {
#ifdef NXPBUILD__PHBAL_REG_STUB
