                                   ...
                                   ) 
{
    ...
    if ((bAtqbLen != 12) && (bAtqbLen != 13)) {
        printf("phpalI14443p3b_Sw_Attrib err1\n\r");
        return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_ISO14443P3B);
    }	  */

    /* check given DSI, DRI, FSDI */
    if ((bFsdi > 0x08) || (bDri > 0x03) || (bDsi > 0x03)) {
       // printf("phpalI14443p3b_Sw_Attrib err2\n\r");
        return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_ISO14443P3B);
    }
...
//  aCmd[8] = bCid;
    ...
    // dwFwt = 70 + (uint32_t)(330.4f * (1 << pDataParams->bFwi));
    dwFwt = 100 + (uint32_t)(330.4f * (1 << pDataParams->bFwi));
    ...
    * 设定 帧的等待时间
    ***************************************************************/
//    printf("phpalI14443p3b_Sw_Attrib 设定 帧的等待时间\n\r");
//  if (bAtqbLen == 13) {
//
//      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(
//                                                       pDataParams->pHalDataParams,
//                                                       PHHAL_HW_CONFIG_TIMEOUT_VALUE_US,
//                                                       (unsigned int)(560*9.44f)));//
//
//
//  } else
     {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Exchange(
                                                    ...
                                                    &pResp,
                                                    &wRespLength));
//    printf(" phpalI14443p3b_Sw_Attrib statusTmp = %02x\n\r",statusTmp);
...
//      return PH_ADD_COMPCODE(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P3B);
//  }
    if ((pResp[0] & 0x0F) != bCid) {//PBOC //TA306.10,TA306.11
//        printf("接收到了非法的CID = %02X\n\r",bCid);
        return PH_ADD_COMPCODE(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P3B);
    }
...
//  }
    ...
    pDataParams->bCid = 0;
    pDataParams->bFsci = ((pAtqb[10] >> 4)>8)?8:((pAtqb[10] >> 4));
...
//  }
    ...
    if (bAtqbLen == 13) {
        ...
        dwSfgt = (uint32_t)(330.4f * (1 << bSfgi));
...
//  else
    {
        phhalHw_Wait(pDataParams->pHalDataParams,PHHAL_HW_TIME_MICROSECONDS,600);//保护时间 最小为0.5MS
                                         ...
                                         )
{
    ...
    *pMoreCardsAvailable = 0x00;
    ...
    /* Request/WakeUpB */
    if (bPupiLength != 0) {
        status = phpalI14443p3b_Sw_WakeUpB(pDataParams, bNumSlots, bAfi, bExtAtqb, pLocalAtqb, &bLocalAtqbLen);
    } else {
        status = phpalI14443p3b_Sw_RequestB(pDataParams, bNumSlots, bAfi, bExtAtqb, pLocalAtqb, &bLocalAtqbLen);
    }
    ...
    for ( bSlot = 2; ; bSlot++ ) {
        ...
        status = phpalI14443p3b_Sw_SlotMarker(pDataParams, bSlot, pLocalAtqb, &bLocalAtqbLen);
    }
    ...
    *pAtqbLen = bLocalAtqbLen;
    ...
    status = phpalI14443p3b_Sw_Attrib(pDataParams, pAtqb, *pAtqbLen, bFsdi, bCid, bDri, bDsi, pMbli);
                                        ...
                                        )
{
    ...
    bExtAtqb = pDataParams->bExtAtqb;//扩张为

    /* Response check */
//  if (((wRespLength != 12) && (wRespLength != 13)) ||
//      ((wRespLength == 13) && (bExtAtqb == PH_OFF))) {//atqb发送提示不支持扩展的，结果返回的扩展的，所以就错误
//      return PH_ADD_COMPCODE(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P3B);
//  }

//屏蔽EMV检测
  //  if ((wRespLength != 12) && (wRespLength != 13)) {
 //       printf("ATQB返回的长度错误  = %d\n\r",wRespLength);
...
//    }
    ...
    *pAtqbLen = (uint8_t)wRespLength;
    ...
    /* max frame size integer < 0x09 *///如果大于8，那么就为 8
...
phStatus_t phpalI14443p3b_Sw_RequestBEx(
                                       ...
                                       uint8_t bNumSlots,
                                       uint8_t bAfi,
                                       uint8_t bExtAtqb,//是否使能扩展参数 和时间槽共用的一个参数
                                       uint8_t * pAtqb,
                                       uint8_t * pAtqbLen
                                       ){
    phStatus_t  PH_MEMLOC_REM statusTmp;
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Exchange(
                                                    ...
                                                    &pResp,
                                                    &wRespLength));
//    Print_LogHex("phpalI14443p3b_Sw_RequestBEx",&pResp[0],wRespLength);
...
/**************************************************************
*额外使用时间为1ms,因为发送数据的时间大概要1ms,对于这个命令
***************************************************************/
phStatus_t phpalI14443p3b_Sw_RequestBExAddTime(
                                              phpalI14443p3b_Sw_DataParams_t * pDataParams,
                                              uint8_t bIsWakeUp,
                                              uint8_t bNumSlots,
                                              uint8_t bAfi,
...
//phStatus_t phpalI14443p3bEMV_Sw_ActivateCard(ContextInfo *ReaderContextInfo,
//                                             phpalI14443p3b_Sw_DataParams_t * pDataParams,
//                                             uint8_t * pPupi,
//                                             uint8_t  bPupiLength,
//                                             uint8_t bNumSlots,
...
//
//
//    if ((ReaderContextInfo->TypeBFlag==0)||(ReaderContextInfo->TypeBFlag==1)) {
//        DelayMs(6);
//        /* Request/WakeUpB ,标准使用WAKE UP*/
//        if (ReaderContextInfo->TypeBFlag==0) {
//            //一共使用4.4ms
//            status =  phpalI14443p3b_Sw_RequestBExAddTime(pDataParams,
//                                                          1,
//                                                          0,//bNumSlots,
//                                                          0,//bAfi,
//                                                          PH_OFF,
//                                                          pLocalAtqb, &bLocalAtqbLen,
//                                                          PHHAL_HW_TIME_MICROSECONDS, (350*9.44f) //3.3ms
//                                                         );
//
//            if ((status & PH_ERR_MASK)!=PH_ERR_IO_TIMEOUT) {
//
//                // phpalI14443p3b_Sw_HaltB(pDataParams);
//                //如果A B卡同时存在，就场复位 6MS
//                if (ReaderContextInfo->TypeAFlag) {//TA303
//
//                    //PCD帧的延时时间，最小是6780/f = 500us
//                    phhalHw_Wait(pDataParams->pHalDataParams,PHHAL_HW_TIME_MICROSECONDS,(70*9.44f));
//                    status =  phhalHw_Rc663_FieldReset(pDataParams->pHalDataParams);
//                    printf("polling B卡的时候，发现已经有A卡\n\r");
//                    ReaderContextInfo->TypeAFlag = 0;
//                    ReaderContextInfo->TypeBFlag = 0;
//                } else {
//                    ReaderContextInfo->TypeBFlag = 1;
//                }
//                //printf("phpalI14443p3bEMV_Sw_RequestAEx= %02x\n\r",(status & PH_ERR_MASK));
//                return status;
//            } else {
//                // printf()
...
//                                                              PH_OFF,
//                                                              pLocalAtqb, &bLocalAtqbLen,
//                                                              PHHAL_HW_TIME_MICROSECONDS, (180*9.44f)
//                                                             );
//                if ((status&PH_ERR_MASK)==PH_ERR_SUCCESS) {
//                    phhalHw_Wait(pDataParams->pHalDataParams,PHHAL_HW_TIME_MICROSECONDS,(uint16_t)(70*9.44f));//
//                    ReaderContextInfo->TypeBFlag = 2;
//
//                    break;
//
//
//                }
//                //
//                if (((i==2)&&((status&PH_ERR_MASK)!=PH_ERR_SUCCESS))||
//                    (((status&PH_ERR_MASK)!=PH_ERR_SUCCESS)&&((status&PH_ERR_MASK)!=PH_ERR_IO_TIMEOUT))) {//非超时的错误
//                    phhalHw_Wait(pDataParams->pHalDataParams,PHHAL_HW_TIME_MICROSECONDS,(uint16_t)(70*9.44f));//
//                    phhalHw_Rc663_FieldReset(pDataParams->pHalDataParams);
//                    ReaderContextInfo->TypeAFlag = 0;
//                    ReaderContextInfo->TypeBFlag = 0;
//                    printf("POLLING B  tow  err status = %02x\n\r",(status&PH_ERR_MASK));
//
//                    return status;
//                }
//            }
//            //printf("phpalI14443p3b_Sw_Attrib start\n\r");
//
//            memcpy(pAtqb, pLocalAtqb, bLocalAtqbLen ); /* PRQA S 3200 */
//            *pAtqbLen = bLocalAtqbLen;
//            //设置位速率
...
//                    ReaderContextInfo->TypeAFlag = 0;
//                    ReaderContextInfo->TypeBFlag = 3;
//                    break;
//                    //只要不是超时的错误，就复位
//                } else if (((status&PH_ERR_MASK)!=PH_ERR_IO_TIMEOUT)||
//                           ((i==3)&&((status&PH_ERR_MASK)!=PH_ERR_SUCCESS))) {
//                    phhalHw_Wait(pDataParams->pHalDataParams,PHHAL_HW_TIME_MICROSECONDS,(uint16_t)(70*9.44f));//
//                    phhalHw_Rc663_FieldReset(pDataParams->pHalDataParams);
//                    ReaderContextInfo->TypeAFlag = 0;
//                    ReaderContextInfo->TypeBFlag = 0;
//                    printf("Sw_Attrib status= %x\n\r",status);
//                    return status ;
//                }
////              if ((status & PH_ERR_MASK) != PH_ERR_SUCCESS) {
////                  /* PUPI is invalid */
...
//phStatus_t  phpal14443p3bEMV_SW_RemoveCard(ContextInfo *ReaderContextInfo , void * pDataParams){
//    phStatus_t  PH_MEMLOC_REM status;
//    uint8_t  bAtqa[2];g
//    uint8_t loop;
//
...
//
//
//    if (ReaderContextInfo->TypeBFlag) {
//        phhalHw_Rc663_FieldOff( ((phpalI14443p3b_Sw_DataParams_t *)pDataParams)->pHalDataParams);
//        DelayMs(6);
//        phhalHw_Rc663_FieldOn( ((phpalI14443p3b_Sw_DataParams_t *)pDataParams)->pHalDataParams);
//        DelayMs(6);
//        loop=1;
//        for ( ;loop<=3;loop++) {
//            if (loop<=2) {
...
//                                                              PH_OFF,
//                                                              pLocalAtqb, &bLocalAtqbLen,
//                                                              PHHAL_HW_TIME_MICROSECONDS, (738*9.44f)
//                                                             );
//                if ((status & PH_ERR_MASK)!=PH_ERR_IO_TIMEOUT) {
//                    DelayMs(6);
//                    loop = 0;
...
//                                                              PH_OFF,
//                                                              pLocalAtqb, &bLocalAtqbLen,
//                                                              PHHAL_HW_TIME_MICROSECONDS, (50*9.44f)
//                                                             );
//                if ((status & PH_ERR_MASK)!=PH_ERR_IO_TIMEOUT) {
//                    DelayMs(6);
//                    loop = 0;
...
//            printf("移除成功\n\r");
//        }
//        ReaderContextInfo->TypeBFlag = 0;//空闲
//        return  0;
//    }
//}
