...
#define PHHAL_HW_RC663_OPTION_RXTX_TIMER_START          0x0010U
	...
	*/
	typedef struct
	{
		...
		uint16_t wTxBufSize;//发送缓冲区的大小                /**< Size of the global transmit buffer. */
		uint16_t wTxBufLen; //发送缓冲区有效字节的长度       /**< Number of valid bytes within the transmit buffer. */
		uint8_t * pRxBuffer;                                /**< Pointer to global receive buffer used by the Exchange() function. */
		uint16_t wRxBufSize; //缓冲区的大小             /**< Size of the global receive buffer. */
		uint16_t wRxBufLen; //接收缓冲区的有效字节个数    /**< Number of valid bytes within the receive buffer. */
		uint16_t wRxBufStartPos; //在整个接收缓冲区的位置的位置,在PICC 链接块中，每次收到了一个I块，wRxBufStartPos就++,
...
//  {PHHAL_HW_CONFIG_ASK100,                PH_ON}
		...
		uint16_t wTimingMode;                               /**< 定时模式. */

		uint8_t bTimeoutUnit; //保存  设置 FWT设置时间单位                 /**< Unit of current timeout value (either #PHHAL_HW_TIME_MICROSECONDS or #PHHAL_HW_TIME_MILLISECONDS). */
		uint32_t dwTimingUs;  //保存FWT使用的时间，单位为US，PCD和PICC之间的交互时间                         
       ...
       //关闭电磁场的时间
		...
		//如果是和PICC交互完成，那么wAdditionalInfo保存最后接收的BIT个数，0标示完整
		...
		uint8_t bRfResetAfterTo;                            /*是否溢出自动复位，默认为关闭*/
		//
		uint8_t bDateFliter;//0--禁止过虑干扰；1--使能过硫
...
#define PHHAL_HW_CONFIG_TIMEOUT_VALUE_US    0x000DU     /**< Set RC Timeout (in [us]). *///设置FWT时间，同是在showderCfg保存 bTimeoutUnit
#define PHHAL_HW_CONFIG_TIMEOUT_VALUE_MS    0x000EU     /**< Set RC Timeout (in [ms]). *///设置FWT时间,单位保存在bTimeoutUnit
#define PHHAL_HW_CONFIG_SUBCARRIER          0x000FU     /**< Subcarrier setting for ISO 15693. */
#define PHHAL_HW_CONFIG_TIMING_MODE         0x0010U     /**< Set the timing mode. *///设置定时模式
#define PHHAL_HW_CONFIG_TIMING_US           0x0011U     /**< Retrieve elapsed time of RC timer ([us]). */
//获取FDT使用的时间，之后pDataParams->dwTimingUs 清 0
...
#define PHHAL_HW_FIELD_OFF_DEFAULT          0x0006U     /**< Default Field-Off time. */
#define PHHAL_HW_FIELD_RECOVERY_DEFAULT     0x0001U     /**< Default Field-Recovery time. */
	/*@}*/
