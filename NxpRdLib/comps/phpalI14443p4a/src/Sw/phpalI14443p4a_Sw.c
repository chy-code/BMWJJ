...
//phStatus_t phpalI14443p4aEMV_Sw_Rats(ContextInfo *ReaderContextInfo,
//                                     phpalI14443p4a_Sw_DataParams_t * pDataParams,
//                                     uint8_t bFsdi,//PCD 能接收最大帧的大小
//                                     uint8_t bCid,
//                                     uint8_t * pAts
//                                    ){
...
//                                                     (unsigned int)(560*9.44f)));//TA003
////      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(
////                                                   pDataParams->pHalDataParams,
////                                                   PHHAL_HW_CONFIG_TIMEOUT_VALUE_US,
////                                                   (unsigned int)(1000*9.44f)));//TA003
//
//
//
//
...
//            //PICC能接收最大帧的尺寸,当FSCI 的值为‘9’-‘F’时，PCD 应按照FSCI 为‘8’处理
...
//        //dwSfgt = (uint32_t)(PHPAL_I14443P4A_SW_FWT_MIN_US * (1 << bSfgi));//ISO
//        dwSfgt = (uint32_t)(330.4f * (1 << bSfgi)); //EMV 35ETU*2^n
//
//        /* Perform SFGT Wait */
//        if (dwSfgt > 0xFFFF) {
...
//        //dwFwt = (uint32_t)(PHPAL_I14443P4A_SW_FWT_MIN_US * (1 << pDataParams->bFwi));  //ISO14443
//        dwFwt = (uint32_t)(330.4f * (1 << pDataParams->bFwi));//EMV 2.0
//
//        /* Add extension time */
...
//phStatus_t phpalI14443p4aEMV_Sw_RatsDF(ContextInfo *ReaderContextInfo,
//                                       phpalI14443p4a_Sw_DataParams_t * pDataParams,
//                                       uint8_t bFsdi,//PCD 能接收最大帧的大小
//                                       uint8_t bCid,
//                                       uint8_t * pAts
...
//        status = phpalI14443p4aEMV_Sw_Rats( ReaderContextInfo,
//                                            pDataParams,
//                                            bFsdi,//PCD 能接收最大帧的大小
//                                            bCid,
//                                            pAts);
//        //printf("RatsDF = %02x\n\r",status);
//        if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS) {
//            PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_GetFdt((phhalHw_Rc663_DataParams_t *)pDataParams->pHalDataParams, status, &dwTimingSingle));
//            if (dwTimingSingle!=7) {
//                ReaderContextInfo->TypeAFlag = 4;
//                break;
//            } else {
//                printf("phpalI14443p4aEMV_Sw_RatsDF %d;\n\r",dwTimingSingle);
//            }
//        } else if (((status & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT)||
//                   (((status & PH_ERR_MASK) != PH_ERR_SUCCESS)&&(loop==3))) {
//            printf("phpalI14443p4aEMV_Sw_RatsDF 协议错误\n\r");
//            //如果A B卡同时存在，就场复位 6MS
