                                      ...
                                      )
{
    ...
    while (bLoop < pDataParams->sTypeATargetInfo.bLoopLimit_A) {
        status = phpalI14443p3a_Sw_WakeUpA(
        ...
        if ((status & PH_ERR_MASK) == PH_ERR_COLLISION_ERROR) {
            ...
            //Print_LogHex("phacDiscLoop_Sw_Int_DetectA",aAtqa,2);
            // printf("phacDiscLoop_Sw_Int_DetectA 0K")
...
* º¯Êý£ºï¼š                                                     
* ²ÎÊý£º
* ËµÃ÷£º·À³åÍ»¼ì²â¿¨Æ¬£¬Èç¹û´æÔÚ¶à¸ö¿¨Æ¬£¬¼ì²âÒ»¸öÖ®ºó£¬¾Í·¢ËÍHAULTÃüÁî£¬ÔÚ³¬ÕÒÆäËûµÄ¿¨Æ¬
* ·µ»Ø£ºbTotalTagsFound_A ²éÕÒµ½µÄ¿¨Æ¬ÊýÁ¿£¬Ó°ÏìI3P3µÄ½á¹¹Ìå
***************************************************************/
phStatus_t phacDiscLoop_Sw_Int_CollisionResolutionA(phacDiscLoop_Sw_DataParams_t * pDataParams)
{
        ...
        if (bTypeATagIdx < PHAC_DISCLOOP_TYPEA_I3P3_NR) {
            ...
            for (bCascadeCodeIdx=0; bCascadeCodeIdx<3; bCascadeCodeIdx++) {
                ...
                while (bTypeANvbUid != 0x40) {
                    status = phpalI14443p3a_Sw_Anticollision(
                                                            pDataParams->pPal1443p3aDataParams,
                ...
                status = phpalI14443p3a_Sw_Select(
                                                 pDataParams->pPal1443p3aDataParams,
                                                 aCascadeCodes[bCascadeCodeIdx],                                   /* Cascade code */
                                                 aTypeAUid,
            ...
            if (status == PH_ERR_SUCCESS) {
                ...
                /* Halt the card */
                status = phpalI14443p3a_Sw_HaltA(pDataParams->pPal1443p3aDataParams);
                if (status == PH_ERR_SUCCESS) {
                    /* Move onto next card */
                    bTypeATagIdx++;
                ...
                status = phpalI14443p3a_Sw_RequestA(pDataParams->pPal1443p3aDataParams,aAtqa);

                if (status != PH_ERR_SUCCESS) {
                    /* Cannot proceed with the Anticollision if REQA fails */
                    return status;
                } else {
                    memcpy(pDataParams->sTypeATargetInfo.aTypeA_I3P3[bTypeATagIdx].aAtqa,aAtqa,2);
                }
            }
        }
...
/**************************************************************
*²ÎÊý£º 
*  bTypeATagIdx--¼¤»î¿¨Æ¬µÄË÷Òý£¬Èç¹û´æÔÚ¶à¸ö¿¨Æ¬£¬Ö»ÄÜ¼¤»îÒ»¸ö
*  ËµÃ÷£º
*    ¼¤»î³É¹¦Ö®ºó£¬sTypeATargetInfo.bActivatedTagNumber_A++
***************************************************************/
                                         ...
                                         )
{
    ...
    uint8_t      PH_MEMLOC_REM bSak,bAtq[8];
    ...
    status = phpalI14443p3a_Sw_ActivateCard(
                                           pDataParams->pPal1443p3aDataParams,
                                           pDataParams->sTypeATargetInfo.aTypeA_I3P3[bTypeATagIdx].aUid,
    ...
    if (status == PH_ERR_SUCCESS) {
        ...
        //if (bSak == 0x20 || bSak == 0x24 || bSak == 0x28) {
        if ((bSak&0x24)==0x20) {
            status = phpalI14443p4a_Sw_ActivateCard(
            ...
            phpalI14443p4_Sw_SetProtocol( pDataParams->pPal1443p4DataParams,0,0,0,0,
                                          pDataParams->pPal1443p4aDataParams->bFwi,
                                          pDataParams->pPal1443p4aDataParams->bFsdi,
                                          pDataParams->pPal1443p4aDataParams->bFsci);
            /* Type A tag activated */
            pDataParams->sTypeATargetInfo.bActivatedTagNumber_A++;
            //Print_LogHex("aAts:",pDataParams->sTypeATargetInfo.sTypeA_I3P4.aAts,pDataParams->sTypeATargetInfo.sTypeA_I3P4.aAts[0]);
        }
    ...
    if (status == PH_ERR_SUCCESS) {
        pDataParams->sTypeATargetInfo.aTypeA_I3P3[bTypeATagIdx].bTagState = PHAC_DISCLOOP_TYPEA_STATE_ACTIVATED;
    } else {
        pDataParams->sTypeATargetInfo.aTypeA_I3P3[bTypeATagIdx].bTagState = PHAC_DISCLOOP_STATE_UNKNOWN;
    }
...
phStatus_t phacDiscLoop_Sw_Int_DetectB( phacDiscLoop_Sw_DataParams_t * pDataParams )
{
    ...
    while (bLoop <  pDataParams->sTypeBTargetInfo.bLoopLimit_B) {
        status = phpalI14443p3b_Sw_RequestB(
        ...
        if (status == PH_ERR_INTEGRITY_ERROR) {
            ...
            memcpy( pDataParams->sTypeBTargetInfo.aI3P3B[bTypeBTagIdx].aPupi,
                    &aAtqb[1],4 );
            memcpy( pDataParams->sTypeBTargetInfo.aI3P3B[bTypeBTagIdx].aAtqB,
                    &aAtqb[0], bAtqbLen);
//            Print_LogHex("phacDiscLoop_Sw_Int_DetectB", &aAtqb[1],1);
        ...
        while (bNumOfSlots <= PHAC_DISCLOOP_TYPEB_MAX_SLOT_NUM) {
            ...
            while (bCurrentSlotNum <= (1 << bNumOfSlots)) {
                ...
                if (bCurrentSlotNum != 0) {
                    ...
                    status = phpalI14443p3b_Sw_SlotMarker(
                                                         pDataParams->pPal1443p3bDataParams,
                                                         bCurrentSlotNum,
                                                         aAtqb,
                                                         &bAtqbLen
...
//                            );
                        ...
                        memcpy( pDataParams->sTypeBTargetInfo.aI3P3B[pDataParams->sTypeBTargetInfo.bTotalTagsFound_B].aPupi,
                                &aAtqb[1], 4 );
                        memcpy( pDataParams->sTypeBTargetInfo.aI3P3B[pDataParams->sTypeBTargetInfo.bTotalTagsFound_B].aAtqB,
                                &aAtqb[0], bAtqbLen );
                        ...
                        if (pDataParams->sTypeBTargetInfo.bTotalTagsFound_B
                            ...
                            status = phpalI14443p3b_Sw_HaltB(
                                                            pDataParams->pPal1443p3bDataParams
                                                            );
                            if (status == PH_ERR_SUCCESS) {
...
//                                );
                            memcpy( pDataParams->sTypeBTargetInfo.aI3P3B[pDataParams->sTypeBTargetInfo.bTotalTagsFound_B].aPupi,
                                    &aAtqb[1], 4);
                            memcpy(
                                  pDataParams->sTypeBTargetInfo.aI3P3B[pDataParams->sTypeBTargetInfo.bTotalTagsFound_B].aAtqB,
                                  &aAtqb[0],bAtqbLen);
                            pDataParams->sTypeBTargetInfo.aI3P3B[pDataParams->sTypeBTargetInfo.bTotalTagsFound_B].bAtqBLength = bAtqbLen;
                            pDataParams->wTagsFound |= PHAC_DISCLOOP_TYPEB_DETECTED;
                            pDataParams->sTypeBTargetInfo.bTotalTagsFound_B++;
...
/**************************************************************
* 
* ËµÃ÷£ºÓ°ÏìpDataParams->sTypeBTargetInfo.bActivatedTagNumber_B++;
***************************************************************/
                                        ...
                                        uint8_t                        bTypeBTagIdx)
{
    ...
    if (status == PH_ERR_SUCCESS) {
        ...
        phpalI14443p4_Sw_SetProtocol(pDataParams->pPal1443p4DataParams,0,0,0,0,
                                     pDataParams->pPal1443p3bDataParams->bFwi,
                                     pDataParams->pPal1443p3bDataParams->bFsdi,
                                     pDataParams->pPal1443p3bDataParams->bFsci);
...
/**************************************************************
* 
* ËµÃ÷£ºÖ´ÐÐ´Îº¯Êý£¬Ö»ÄÜpDataParams->sTypeFTargetInfo.bTotalTagsFound_F = 1£¬²»ÄÜÍ¬Ê±·¢ÏÖ¶à¸ö
*      F¿¨±êÇ©
***************************************************************/
...
//          Buzzer_OFF();
            ...
            //  Print_LogHex("phpalFelica_ReqC",aRxBuf,16);
