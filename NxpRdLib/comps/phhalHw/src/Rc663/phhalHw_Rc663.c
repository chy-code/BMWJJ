...
#define PROCESS   1 //0不采用任何 ；1--采用中断 2--采用查询错误寄存器 
unsigned char aGetInputBufStr[1024];
unsigned char aGetInputBufData[256];
int aGetInputBufDataLen = 0;

extern ContextInfo ReaderISOPare;//读写器参数

extern void Print_LogHex(void *str0,unsigned char *hex,unsigned int hexlen);
...
static const uint16_t PH_MEMLOC_CONST_ROM wRc663_DefaultShadow_I14443a[][2] =
{
    ...
    //{PHHAL_HW_CONFIG_TXWAIT_US,             81},   //发送等待时间，在这段时间内，忽视所以的接收的数据
    {PHHAL_HW_CONFIG_TXWAIT_US,             36},
                             ...
                             )
{
    ...
    pDataParams->bDateFliter            = PH_OFF;
...
phStatus_t phhalHw_Rc663_ExchangeISO(
                                    phhalHw_Rc663_DataParams_t * pDataParams,
                                    uint16_t wOption,
                                    uint8_t * pTxBuffer,
                                    uint16_t wTxLength,
                                    ...
                                    )
{
    ...
    uint8_t     PH_MEMLOC_REM bError,wNbyte;
    ...
    // Print_LogHex("Rc663_Exchange send :",pTmpBuffer,wTmpBufferLen);
    IWDG_ReloadCounter();
...
//  if (status == PH_ERR_SUCCESS) {
//      printf("EXCHANGE REV DATA bError:%02x;status:%02x\n\r",bError,status);
//
//      for (wNbyte = 0; wNbyte< pDataParams->wRxBufLen; wNbyte++)
//          printf("%02x ",pDataParams->pRxBuffer[wNbyte]);
//      printf("\n\r");
//  }
...
phStatus_t phhalHw_Rc663_Exchange(
    ...
    return phhalHw_Rc663_ExchangeISO(
                                    pDataParams,
                                    wOption,
                                    pTxBuffer,
                                    wTxLength,
...
//    uint8_t     PH_MEMLOC_REM bIrq0WaitFor;
//    uint8_t     PH_MEMLOC_REM bIrq1WaitFor;
//    uint8_t     PH_MEMLOC_REM bIrq0Reg = 0x00;//如果接收到了数据，IDEIrq   RxIrq都会置1
//    uint8_t     PH_MEMLOC_REM bIrq1Reg = 0x00;
//    uint8_t     PH_MEMLOC_REM bRegister;
...
//    uint8_t *   PH_MEMLOC_REM pTmpBuffer;
//    uint16_t    PH_MEMLOC_REM wTmpBufferLen;
//    uint16_t    PH_MEMLOC_REM wTmpBufferSize ,wT1Reload;
//
//    uint32_t    dwTimeus;
//    uint16_t    wNbyte;
//    uint16_t    wNBitAll;
//    uint8_t     bControlReg,bBit,bRegRxWait,bRegTxWait,bRegRxCtl;
//    unsigned    char userstatus;
//    uint8_t     bRetryFlag = 0;
//    uint8_t i ;
//
//
//
//    //if (ReaderISOPare.TypeAFlag==0x03)
////  {
////      LEDGreen_OFF();
////  }
//
...
//            memcpy(&pTmpBuffer[wTmpBufferLen], pTxBuffer, wTxLength);  /* PRQA S 3200 */
//            pDataParams->wTxBufLen = pDataParams->wTxBufLen + wTxLength;
//            // printf("rc663  发送缓冲区的大小 = %d\n\r",pDataParams->wTxBufLen);
//        }
//
//        /* Buffer operation -> finished */
//        if (wOption & PH_EXCHANGE_BUFFERED_BIT) {
...
//
//
//    //printf("\n\r\n\rrphhalHw_Rc663_Exchange start\n\r");
//
//    /* retrieve transmit buffer */
//    phhalHw_Rc663_GetTxBuffer(pDataParams, PH_ON, &pTmpBuffer, &wTmpBufferLen, &wTmpBufferSize);
//
//    /* Disable DataEn if no data is to be sent */
//    if ((wTmpBufferLen == 0) && (wTxLength == 0)) {
//        /* Store that we sent no data */
...
//
//        /* Read out TxDataNum register */
//        phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_TXDATANUM, &bRegister);
//
//        /* Disable DataEn */
//        bRegister &= (uint8_t)~(uint8_t)PHHAL_HW_RC663_BIT_DATAEN;
//
//        /* Write changed register */
//        phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TXDATANUM, bRegister);
//    } else {
//
//        /* Reset NoData flag */
//        bNoData = PH_OFF;
...
//    /*   设置中断条件和超时时间*/
//    bIrq0WaitFor = PHHAL_HW_RC663_BIT_TXIRQ | PHHAL_HW_RC663_BIT_IDLEIRQ;
//    bIrq1WaitFor = PHHAL_HW_RC663_BIT_TIMER1IRQ;//定时器是发送自动启动，接收自动停止
//
//
//
...
////  }
////
////  if ( ReaderISOPare.TypeAFlag==PROFLAG) {
////      IAP_SET_L();
////  }
//
//    phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_RXCTRL, &bRegRxCtl);
//    phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_RXWAIT, &bRegRxWait);
//    /*--------------第一次接收 起始位或者溢出报警--------------*/
//
//    if (!(bIrq0Reg & PHHAL_HW_RC663_BIT_IDLEIRQ) && !(bIrq0Reg & PHHAL_HW_RC663_BIT_ERRIRQ)) {
//
//        /* Clear high-alert irq flag */
//        phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ0, PHHAL_HW_RC663_BIT_HIALERTIRQ);
//
//        Addr_RetryRev1:
//
//        /* 设置中断条件  检测到SOF信号、数据溢出报警、命令更改或空闲（命令寄存器改变），接收数据结束*/
//        bIrq0WaitFor = PHHAL_HW_RC663_BIT_RXIRQ | PHHAL_HW_RC663_BIT_HIALERTIRQ | PHHAL_HW_RC663_BIT_IDLEIRQ | PHHAL_HW_RC663_BIT_EMDIRQ;
//
//
//        /* 等待接收 ，如果有数据，完成之后，返回0x7c ，改变了IDEIqr,RxIrq*/
//
...
//
//        /* 立即停止定时器 */
//        phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_TCONTROL, &bRegister);
//        /* manually stop timers */
//        bRegister |= PHHAL_HW_RC663_BIT_T0STARTSTOPNOW | PHHAL_HW_RC663_BIT_T1STARTSTOPNOW;
//        bRegister &= (uint8_t)~(uint8_t)(PHHAL_HW_RC663_BIT_T0RUNNING | PHHAL_HW_RC663_BIT_T1RUNNING);
//        phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TCONTROL, bRegister);
//
//        phhalHw_Rc663_GetFdt(pDataParams, status, &dwTimingSingle);
//
//        /* Reset after timeout behaviour */
//        if (pDataParams->bRfResetAfterTo != PH_OFF) {
//            phhalHw_Rc663_FieldReset(pDataParams);
//        }
//
////      if (ReaderISOPare.TypeAFlag==PROFLAG) {//检测到错误到出干扰55us
////          IAP_SET_H();
////      }
//
//        if (bRetryFlag==1) {
//            printf("过滤干扰 收到数据超时;bIrq0Reg = %02x;bIrq1Reg = %02x;\n\r",bIrq0Reg,bIrq1Reg);
//        } else {
////          if (ReaderISOPare.TypeAFlag==1) {
////               printf("  第1次超时 = %dus ;bIrq0Reg = %02x;bIrq1Reg = %02x ;\n\r",dwTimingSingle,bIrq0Reg,bIrq1Reg);
////          }
//            if (DebugPinErr()) {//bIrq0Reg = 0x28
//
//                printf("  第1次超时 = %dus ;bIrq0Reg = %02x;bIrq1Reg = %02x ;ReaderISOPare.TypeAFlag = %02X;\n\r",dwTimingSingle,bIrq0Reg,bIrq1Reg,ReaderISOPare.TypeAFlag);
//            }
//        }
//
//        status = PH_ERR_IO_TIMEOUT;
...
//    pDataParams->wRxBufLen = 0;
//    /* Retrieve receive buffer properties */
//    phhalHw_Rc663_GetRxBuffer(pDataParams, PH_ON, &pTmpBuffer, &wTmpBufferLen, &wTmpBufferSize);
//
//    /*--------------第二次接收 起始位或者溢出报警--------------*/
//    if (status == PH_ERR_SUCCESS) {
//        /* 屏蔽溢出报警 */
...
//        /*循环读数据，只要缓冲区不为0或者中断*/
//
//        /* 检测到SOF信号、命令更改或空闲（命令寄存器改变），接收数据结束*/
//
//        /* retrieve fifo bytes */
//        if (ReaderISOPare.TypeAFlag==PROFLAG) {//从监测到错误到干扰处理结束用50us的时间
//            LEDRed_ON();
//        }
//        while (1) {
//            /* read interrupt status */
//            LEDRed_ON();
//            if (bIrq0Reg&PHHAL_HW_RC663_BIT_HIALERTIRQ) {//高报警
//                phhalHw_Rc663_ReadFifo( pDataParams,wTmpBufferSize,
//                                        &pTmpBuffer[wTmpBufferLen], &wFifoBytes);
//                wTmpBufferLen = wTmpBufferLen + wFifoBytes;
//                wTmpBufferSize = wTmpBufferSize - wFifoBytes;
//                SPI_RC663_Write( (PHHAL_HW_RC663_REG_IRQ0 << 1),PHHAL_HW_RC663_BIT_HIALERTIRQ);//清除标识
//            }
//            LEDRed_OFF();
//            bIrq0Reg = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_IRQ0 << 1) | 0x01U);
//            bIrq1Reg = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_IRQ1 << 1) | 0x01U);
//
//
//            //如果接收到结束位或者超时
//            //if (((bIrq0Reg & bIrq0WaitFor) ||(bIrq1Reg & bIrq1WaitFor)) && (wFifoBytes == 0)) {//
//            if (((bIrq0Reg & bIrq0WaitFor) ||(bIrq1Reg & bIrq1WaitFor))) {//IDE报警，数据接收结束报警
//
//                Addr_RetryRevJust:
//                if (ReaderISOPare.TypeAFlag==PROFLAG) {
//                    LEDRed_ON();
...
//
//
//                //以下到干扰处理结束，55us,之前是220us
//                bError = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_ERROR << 1) | 0x01U);
//
//
//                phhalHw_Rc663_GetFdt(pDataParams, status, &dwTimeus);//20us
//
//                if ((pDataParams->bDateFliter==PH_OFF)||
//                    ((bError==0x00)&&(((unsigned int)(dwTimeus/9.44f))>=9)&&(pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443A) && (bRetryFlag==0))||
//                    ((bError==0x00)&&(((unsigned int)(dwTimeus/9.44f))>=17)&&(pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443B) && (bRetryFlag==0))||
//                    ((bError==0x00)&&(bRetryFlag==1))||
//                    (bIrq1Reg & PHHAL_HW_RC663_BIT_TIMER1IRQ )) {//定时器超时
//
//
//                    i = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_RXCRCCON << 1) | 0x01U);
//                    if ((i&0x80)&&(bError==0x00)) {//如果接收的数据OK，而且CRC时能写入缓冲区
//                        wTmpBufferLen = wTmpBufferLen-2;
//                    }
//
//                    bBit = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_RXBITCTRL << 1) | 0x01U);
//
//                    bBit = bBit&0x07;
//
//
//                    wNbyte = wTmpBufferLen;
//
//                    if (bBit)
//                        wNBitAll = (wNbyte-1) * 8 + bBit;
//                    else
//                        wNBitAll = wNbyte * 8;
//
//                    if (DebugPinRetry()) {
//                        if (DebugPinErr()) {
//                            printf("OK 跳出bRetryFlag = %02x;bError = %02xH;wNbyte = %dD;wNBitAll = %dD;ETU = %d;bBit = %dD;ReaderISOPare.TypeAFlag = %dD;bIrq0Reg = 0X%02X;bIrq1Reg = 0X%02X\n\r",bRetryFlag,bError,wNbyte,wNBitAll,((unsigned int)(dwTimeus/9.44f)),bBit,ReaderISOPare.TypeAFlag,bIrq0Reg,bIrq1Reg);
//                        }
//                    }
//                    if (bRetryFlag==1) {
//                        printf("过滤干扰 OK 跳出bRetryFlag = %02x;bError = %02xH;wNbyte = %dD;wNBitAll = %dD;ETU = %d;bBit = %dD;ReaderISOPare.TypeAFlag = %dD;bIrq0Reg = 0X%02X;bIrq1Reg = 0X%02X\n\r",bRetryFlag,bError,wNbyte,wNBitAll,((unsigned int)(dwTimeus/9.44f)),bBit,ReaderISOPare.TypeAFlag,bIrq0Reg,bIrq1Reg);
//                    }
...
//                    bBit = bBit&0x07;
//                    if (DebugPinRetry())
//                        printf("wTmpBufferLen = %0dD;bBit = %dD\n\r",wTmpBufferLen,bBit);
//
//                    wNbyte = wTmpBufferLen;
//                    if (bBit)
//                        wNBitAll = (wNbyte-1) * 8 + bBit;
...
//                        wNBitAll = wNbyte * 8;
//                    if (DebugPinRetry())
//                        printf("干扰过虑处理 bError = %02xH;wNbyte = %dD;wNBitAll = %dD;ETU = %d;bBit = %dD;ReaderISOPare.TypeAFlag = %dD;TypeBFlag = %dD\n\r;bIrq0Reg = 0X%02X;bIrq1Reg = 0X%02X\n\r",bError,wNbyte,wNBitAll,((unsigned int)(dwTimeus/9.44f)),bBit,ReaderISOPare.TypeAFlag,ReaderISOPare.TypeBFlag,bIrq0Reg,bIrq1Reg);
//
//                    if ((pDataParams->bCardType ==PHHAL_HW_CARDTYPE_ISO14443A)) {
//                        if ((wNbyte>=4)&& (!(wNBitAll%8))&&(((unsigned int)(dwTimeus/9.44f))>=9)) {
//                            printf("A非干扰，禁止容错\n\r");
//                            break;
//                        }
//                    } else {
//                        //对于B卡容错 2013 7 13 上午测试的
////                      if (((wNbyte>=4)&& (!(wNBitAll%8))&&(((unsigned int)(dwTimeus/9.44f))>=16)&&!(bError&PHHAL_HW_RC663_BIT_MINFRAMEERR))||
////                          ((wNbyte==1)&&((wNBitAll%8)==0)&&(ReaderISOPare.TypeBFlag==2)&&((bError&PHHAL_HW_RC663_BIT_INTEGERR)==PHHAL_HW_RC663_BIT_INTEGERR)&&(((unsigned int)(dwTimeus/9.44f))>=16))) {
////                          printf("B非干扰，禁止容错\n\r");
////                          break;
////                      }
//
//                        if (((wNbyte>=6)&& (!(wNBitAll%8))&&(((unsigned int)(dwTimeus/9.44f))>=16)&&!(bError&PHHAL_HW_RC663_BIT_PROTERR)&&!(bError&PHHAL_HW_RC663_BIT_MINFRAMEERR))||//有多余位错误或者SOF EOD错误
//                            ((wNbyte==1)&&((wNBitAll%8)==0)&&(ReaderISOPare.TypeBFlag==2)&&((bError&PHHAL_HW_RC663_BIT_INTEGERR)==PHHAL_HW_RC663_BIT_INTEGERR)&&(((unsigned int)(dwTimeus/9.44f))>=16))) {
//                            printf("B非干扰，禁止容错\n\r");
//                            break;
//                        }
...
//                    wTmpBufferLen =0;
//                    /* Flush FiFo */
//                    phhalHw_Rc663_FlushFifo(pDataParams);
//                    /* clear all IRQ0 flags */
//                    SPI_RC663_Write( (PHHAL_HW_RC663_REG_IRQ0 << 1),(uint8_t)~(uint8_t)PHHAL_HW_RC663_BIT_SET);
//                    /* clear all IRQ1 flags */
//                    SPI_RC663_Write( (PHHAL_HW_RC663_REG_IRQ1 << 1),(uint8_t)~(uint8_t)PHHAL_HW_RC663_BIT_SET);
//                    /* 重新激活接收电路 */
//                    SPI_RC663_Write( (PHHAL_HW_RC663_REG_COMMAND << 1),PHHAL_HW_RC663_CMD_RECEIVE);
//                    //重新启动定时器
//                    bControlReg = SPI_RC663_Read( (uint8_t)(PHHAL_HW_RC663_REG_TCONTROL << 1) | 0x01U);
...
//                    SPI_RC663_Write( (PHHAL_HW_RC663_REG_TCONTROL << 1),bControlReg);
//                    LEDGreen_OFF();
//                    LEDRed_OFF();
//                    goto Addr_RetryRev1;
//
...
//        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_COMMAND, PHHAL_HW_RC663_CMD_IDLE));//停止命令
//
...
//            printf("超时2\n\r");
//            status = PH_ERR_IO_TIMEOUT;
//        } else if (bError & PHHAL_HW_RC663_BIT_NODATAERR) {//没有接收到数据
//            status = PH_ERR_INTERNAL_ERROR;//PH_ERR_IO_TIMEOUT
//        }
//        /* buffer overflow */
//        else if (bError & PHHAL_HW_RC663_BIT_FIFOOVL) {//缓冲区溢出
//            status = PH_ERR_BUFFER_OVERFLOW;
//        }
//        /* Fifo write error */
//        else if (bError & PHHAL_HW_RC663_BIT_FIFOWRERR) {//
...
//        }
//        /* 发生了碰撞 */
//        else if (bError & PHHAL_HW_RC663_BIT_COLLDET) {//发生了碰撞
//            /* No bytes received -> timeout */
//            //printf("发生了碰撞\n\r");
//            if (wTmpBufferLen == 0) {
//                status = PH_ERR_IO_TIMEOUT;
                                      ...
                                      )
{
    ...
    SPI_RC663_Write( (bAddress << 1),bValue);
...
{
    *pValue = SPI_RC663_Read( (uint8_t)(bAddress << 1) | 0x01U);
    return PH_ADD_COMPCODE(PH_ERR_SUCCESS, PH_COMP_HAL);
}
                                              ...
                                              )
{
    ...
    if (bCardType != PHHAL_HW_CARDTYPE_CURRENT) {
        ...
        // PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_RXSOFD, 0x20));//打开有效SOF检测
    }
                                  ...
                                  )
{
    ...
    case PHHAL_HW_CONFIG_TXWAIT_US:  //设置帧等待时间 
    ...
    case PHHAL_HW_CONFIG_TIMEOUT_VALUE_MS:
        ...
        /* Write config data into shadow */
        pDataParams->wCfgShadow[wConfig] = wValue;//保存时间寄存器设定数值
        break;
                                   ...
                                   )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_Wait(
                                                      pDataParams,
                                                      PHHAL_HW_TIME_MILLISECONDS,
                                                      pDataParams->wFieldRecoveryTime));//1ms

    return PH_ADD_COMPCODE(PH_ERR_SUCCESS, PH_COMP_HAL);
}
/**************************************************************
*  延时等待
*  参数：bUnit--时间单位，us 和ms
*        wTimeout--等待的时间
***************************************************************/
                             ...
                             )
{
    ...
    uint8_t     bT0ControlReg,bT1ControlReg;
    unsigned char buf[2];
    ...
    //解决死机问题
    phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T0CONTROL, &bT0ControlReg);
    phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_T1CONTROL, &bT1ControlReg);
    phhalHw_WriteRegister(pDataParams,PHHAL_HW_RC663_REG_T0CONTROL,PHHAL_HW_RC663_BIT_TAUTORESTARTED  | PHHAL_HW_RC663_VALUE_TCLK_1356_MHZ);
    phhalHw_WriteRegister(pDataParams,PHHAL_HW_RC663_REG_T1CONTROL,PHHAL_HW_RC663_VALUE_TCLK_T0);
    ...
    //等待中断
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WaitIrq(pDataParams, PH_ON, PH_OFF,PH_OFF,  PH_OFF,0x00, PHHAL_HW_RC663_BIT_TIMER1IRQ, NULL, &bIrqRq));


    //解决死机问题
    phhalHw_WriteRegister(pDataParams,  PHHAL_HW_RC663_REG_T0CONTROL, bT0ControlReg);
    phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_T1CONTROL, bT1ControlReg);

    /* manually stop T0 *///停止定时器0
